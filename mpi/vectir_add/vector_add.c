...
void get_n(int* n_p, int rank, int thread_count, MPI_Comm comm) {
  ...
  check_for_error(local_ok, comm, "get_n",
                  "n should be > 0 and evenly divisible by comm_sz n == %d",*n_p);
}
...
void read_vector(double* local_a, int local_n, int n, int rank,
  ...
  if (rank == 0) {
    ...
    check_for_error(local_ok, comm, fname, "Can't allocate temporary vector");
    for (int i = 0; i < n; i++) {
      scanf("%lf", &a[i]);
    }
...
void allocate_vectors(double** local_a_p, double** local_b_p,
  ...
  *local_c_p = malloc(local_n * sizeof(double));
  if (*local_a_p == NULL || *local_b_p == NULL || *local_c_p == NULL)
    local_ok = 0;
  check_for_error(local_ok, comm, fname, "Can't allocate local vector(s)");
}
